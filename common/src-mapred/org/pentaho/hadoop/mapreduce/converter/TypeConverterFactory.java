...
public class TypeConverterFactory {
  ...
  public static boolean isKettleType( Class<?> type ) {
    return type != null
        && ( CharSequence.class.isAssignableFrom( type ) || Number.class.isAssignableFrom( type )
            || byte[].class.equals( type ) || Boolean.class.equals( type ) || java.sql.Date.class.equals( type ) );
   ...
   * 
   * @param vmi
   *          Value Meta with type information to look up
  ...
  /**
   * Get the class for the supplied name of a writable
   * 
   * @param writableClass
   *          the name of a writable class to get
   * @return the class for the writable
   * @throws ClassNotFoundException
   *           if the named writable can't be found
   */
   ...
   */
  private final Map<Key, ITypeConverter<?, ?>> cache;
   ...
   *           Error instantiating a converter while traversing the list of registered type converters
   */
   ...
   * 
   * @param from
   *          Type this converter can convert from
   * @param to
   *          Type this converter can convert to
   * @param converter
   *          The converter to handle the conversion between {@code from} and {@code to}
   */
   ...
   * @throws TypeConversionException
   *           No converter available for these types
   */
   ...
   * {@link Class} to convert to is obtained through {@link #getJavaClass(org.pentaho.di.core.row.ValueMetaInterface)
   * getJavaClass(vmi)}. If no destination type can be determined from {@code vmi} no converter will be returned.
   * 
   * @param from
   *          Type to convert from
   * @param vmi
   *          Value meta with type information to convert to
