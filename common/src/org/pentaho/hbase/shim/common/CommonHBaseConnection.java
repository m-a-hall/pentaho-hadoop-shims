...
public class CommonHBaseConnection extends HBaseConnection {
  ...
  public List<String> getTableFamiles(String tableName) throws Exception {
    ...
    HTableDescriptor descriptor =
      m_admin.getTableDescriptor(m_bytesUtil.toBytes(tableName));
    ...
    if (p != null) {
      ...
      for (Object key : keys) {
        ...
        if (key.toString().equals(COL_DESCRIPTOR_MAX_VERSIONS_KEY)) {
          ...
          // Look up Compression.Algorithm.valueOf() method via reflection, as
          // the API has changed
          // between Hadoop 1 and 2.
          // Also, valueOf() will throw an IllegalArgumentException if the value
          // is not a legitimate algorithm name
          Class<?> compressionAlgorithmClass = getCompressionAlgorithmClass();
          Method valueOf =
            compressionAlgorithmClass.getMethod("valueOf", String.class);

          // Since we don't know the type of the Algorithm class at compile
          // time, we need to
          // use reflection here to set the Compression Algorithm for the
          // HColumnDescriptor
          Method setCompressionType =
            h.getClass().getMethod("setCompressionType",
          ...
          // Again, need to use reflection to get the BloomType class to call
          // valueOf and then setBloomFilterType
          Class<?> bloomTypeClass = getBloomTypeClass();
          Method valueOf = bloomTypeClass.getMethod("valueOf", String.class);

          Method setBloomFilterType =
            h.getClass().getMethod("setBloomFilterType");
  ...
  public void addColumnToScan(String colFamilyName, String colName,
    ...
    // if the column name is empty then add the entire family
    if (Const.isEmpty(colName)) {
      m_sourceScan.addFamily(m_bytesUtil.toBytes(colFamilyName));
    } else {
  ...
  public void addColumnFilterToScan(ColumnFilter cf, HBaseValueMeta columnMeta,
    ...
    try {
      ...
      if (m_sourceScan.getFilter() == null) {
        ...
        FilterList fl =
          new FilterList(matchAny ? FilterList.Operator.MUST_PASS_ONE
      ...
      if (comp != null) {
        ...
        if (columnMeta.isNumeric()) {
          ...
          if (cf.getSignedComparison()) {
            // custom comparator for signed comparison, specific to each shim
            // due to HBase API changes
            Class<?> deserializedNumericComparatorClass =
              getDeserializedNumericComparatorClass();
            if (columnMeta.isInteger()) {
              Constructor ctor =
                deserializedNumericComparatorClass.getConstructor(
                  boolean.class, boolean.class, long.class);
              if (columnMeta.getIsLongOrDouble()) {
                comparator =
                  ctor.newInstance(columnMeta.isInteger(),
                    columnMeta.getIsLongOrDouble(), num.longValue());
              } else {
                comparator =
                  ctor.newInstance(columnMeta.isInteger(),
                    columnMeta.getIsLongOrDouble(), (long) num.intValue());
              }
            } else {
              Constructor ctor =
                deserializedNumericComparatorClass.getConstructor(
              ...
              if (columnMeta.getIsLongOrDouble()) {
                comparator =
                  ctor.newInstance(columnMeta.isInteger(),
                    columnMeta.getIsLongOrDouble(), num.doubleValue());
              } else {
                comparator =
                  ctor.newInstance(columnMeta.isInteger(),
                    columnMeta.getIsLongOrDouble(), (double) num.floatValue());
              }
            }
          ...
          if (!cf.getSignedComparison()) {
            ...
            Class<?> deserializedNumericComparatorClass =
              getDeserializedNumericComparatorClass();
            Constructor ctor =
              deserializedNumericComparatorClass.getConstructor(boolean.class,
          ...
          Boolean decodedB =
            HBaseValueMeta.decodeBoolFromString(tempEncoded, m_bytesUtil);
          // skip if we can't parse the comparison value
          if (decodedB == null) {
            return;
          }

          Class<?> deserializedBooleanComparatorClass =
            getDeserializedBooleanComparatorClass();
          Constructor ctor =
            deserializedBooleanComparatorClass.getConstructor(boolean.class);
      ...
      if (comparator != null) {
        Constructor<SingleColumnValueFilter> scvfCtor =
          SingleColumnValueFilter.class.getConstructor(byte[].class,
            byte[].class, CompareFilter.CompareOp.class, comparatorClass);
        SingleColumnValueFilter scf =
          scvfCtor.newInstance(family, qualifier, comp, comparator);
  ...
  // TODO - Override this method if necessary! Older HBase versions use
  // WritableByteArrayComparable
  // newer ones (0.95+) use ByteArrayComparable
  public Class<?> getByteArrayComparableClass() throws ClassNotFoundException {
    return Class
      .forName("org.apache.hadoop.hbase.filter.WritableByteArrayComparable");
  }
  ...
  public Class<?> getCompressionAlgorithmClass() throws ClassNotFoundException {
    return Class
      .forName("org.apache.hadoop.hbase.io.hfile.Compression.Algorithm");
  }

  // TODO - Override this method if necessary!
  // Older HBase versions use
  ...
  public Class<?> getBloomTypeClass() throws ClassNotFoundException {
    return Class
      .forName("org.apache.hadoop.hbase.regionserver.StoreFile.BloomType");
  }
  ...
  public Class<?> getDeserializedNumericComparatorClass()
    throws ClassNotFoundException {
    return Class
      .forName("org.pentaho.hbase.shim.common.DeserializedNumericComparator");
  }

  // TODO - Override this method to return the specified class for each shim.
  // Older HBase versions extend WritableByteArrayComparable
  // Newer ones (0.95+) extend ByteArrayComparable
  public Class<?> getDeserializedBooleanComparatorClass()
    throws ClassNotFoundException {
    return Class
      .forName("org.pentaho.hbase.shim.common.DeserializedBooleanComparator");
  }
